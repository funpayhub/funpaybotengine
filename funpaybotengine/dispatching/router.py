...
class Router(BaseRouter):
    def __init__(self, name: str = '') -> None:
        ...
        self.on_sale_status_changed = self._fpbe_mgr(events.SaleStatusChangedEvent)
        ...
        self.on_purchase_status_changed = self._fpbe_mgr(events.PurchaseStatusChangedEvent)
        ...
        self.on_error = self.add_handler_manager(HandlerManager('on_error', 'error'))
    ...
    def _fpbe_mgr(self, event: type[events.Event]) -> HandlerManager:
        manager = HandlerManager(f'on_{event.__event_name__}', event.__event_name__)
